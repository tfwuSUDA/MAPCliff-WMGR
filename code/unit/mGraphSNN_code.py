...
class Fusion_atom_and_bond(nn.Module):
    def __init__(self,args):
        ...
        self.mol_GRUCell = nn.GRUCell(args.fingerprint_dim, args.fingerprint_dim)
        self.mol_align = nn.Linear(2 * args.fingerprint_dim, 1)
        self.mol_attend = nn.Linear(args.fingerprint_dim, args.fingerprint_dim)
        self.dropout = nn.Dropout(p=args.p_dropout)
        self.output = nn.Linear(args.fingerprint_dim, args.output_units_num)
...
class mGraphSNNlayer(nn.Module):
    def __init__(self, args, snn_args, input_dim, output_dim):
        super(mGraphSNNlayer, self).__init__()
        ...
        self.attention_kernel_shape = (2 * self.fingerprint_dim, 1)
        ...
        self.kernels = nn.ParameterList([nn.Parameter(torch.empty(self.kernel_shape)) for _ in range(self.num_attention_heads)])
        self.attention_kernels = nn.ParameterList([nn.Parameter(torch.empty(self.attention_kernel_shape)) for _ in range(self.num_attention_heads)])
        self.epsilons = nn.ParameterList([nn.Parameter(torch.full((1,), 0.6)) for _ in range(self.num_attention_heads)])
        if snn_args.use_bias:
            self.biases = nn.ParameterList([nn.Parameter(torch.zeros(self.fingerprint_dim)) for _ in range(self.num_attention_heads)])
            self.attention_biases = nn.ParameterList([nn.Parameter(torch.zeros(1)) for _ in range(self.num_attention_heads)])
    ...
    def forward(self, atom_neighbor_SNN, x_full_atom_neighbors):
        ...
        for i in range(self.num_attention_heads):
            ...
            attention_matrix = torch.nn.functional.elu(attention_matrix, alpha=1.0)
    ...
    def apply_unit_norm(self):
        for kernel in self.kernels:
            kernel.data = nn.functional.normalize(kernel.data, p=2, dim=0)
        for attention_kernel in self.attention_kernels:
            attention_kernel.data = nn.functional.normalize(attention_kernel.data, p=2, dim=0)
        if self.use_bias:
            for bias in self.biases:
                bias.data = nn.functional.normalize(bias.data, p=2, dim=0)
            for attention_bias in self.attention_biases:
                attention_bias.data = nn.functional.normalize(attention_bias.data, p=2, dim=0)



class mGraphSNN(nn.Module):
    def __init__(self, args, snn_args):
        super(mGraphSNN, self).__init__()
        self.num_layers = args.num_layers
        self.dropout = nn.Dropout(p=args.p_dropout)
        self.dropout_linear = nn.Dropout(p=0.2)
        self.Fusion_atom_and_bond = Fusion_atom_and_bond(args)
        self.layers = nn.ModuleList([])
        self.liner_1 = nn.Linear(args.fingerprint_dim * snn_args.num_attention_heads, args.fingerprint_dim * snn_args.num_attention_heads)
        for i in range(0, self.num_layers):
            self.layers.append(mGraphSNNlayer(args, snn_args, snn_args.num_attention_heads * args.fingerprint_dim, snn_args.num_attention_heads * args.fingerprint_dim))
        self.output1 = nn.Linear(39 + args.fingerprint_dim * (args.num_layers + 1) * snn_args.num_attention_heads, args.fingerprint_dim * snn_args.num_attention_heads)
        self.output2 = nn.Linear(args.fingerprint_dim * snn_args.num_attention_heads, args.output_units_num)

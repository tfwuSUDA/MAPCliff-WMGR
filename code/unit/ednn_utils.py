...
class Merge_Dataset(object):
    def __init__(self, Xs, dl_feature ,Ys):
        self.Xs = torch.tensor(Xs, dtype=torch.float32)
        self.dl_feature = torch.tensor(dl_feature, dtype=torch.float32)
        self.masks = torch.tensor(~np.isnan(Ys) * 1.0, dtype=torch.float32)
        # convert np.nan to 0
        self.Ys = torch.tensor(np.nan_to_num(Ys), dtype=torch.float32)
    ...
    def __getitem__(self, idx):
        ...
        X = self.Xs[idx]
        dl_feature = self.dl_feature[idx]
        Y = self.Ys[idx]
        mask = self.masks[idx]

        return X, dl_feature, Y, mask
...
class MyDNN(nn.Module):
    def __init__(self, inputs, hidden_units, outputs, dp_ratio, reg):
        ...
        self.hidden4 = nn.Linear(hidden_units[2], hidden_units[3])
        self.dropout4 = nn.Dropout(dp_ratio)

        if reg:
            self.output = nn.Linear(hidden_units[3], 1)
        else:
            self.output = nn.Linear(hidden_units[3], outputs)
...
class IFM_DNN_4(nn.Module):
    def __init__(self, inputs, hidden_units, outputs, d_out, sigma, dp_ratio, first_omega_0, hidden_omega_0, reg):
        ...
        self.hidden4 = SineLayer(hidden_units[2], hidden_units[3], is_first=False, omega_0=hidden_omega_0)
        self.dropout4 = nn.Dropout(dp_ratio)

        if reg:
            self.output = nn.Linear(hidden_units[3], 1)
            with torch.no_grad():
                self.output.weight.uniform_(-np.sqrt(6 / hidden_units[3]) / hidden_omega_0, 
                                              np.sqrt(6 / hidden_units[3]) / hidden_omega_0)
        else:
            self.output = nn.Linear(hidden_units[3], outputs)
            with torch.no_grad():
                self.output.weight.uniform_(-np.sqrt(6 / hidden_units[3]) / hidden_omega_0, 
...
class merge_IFM(nn.Module):
    def __init__(self, inputs, hidden_units, outputs, d_out, sigma, dp_ratio, first_omega_0, hidden_omega_0, reg, numpy_dim):
        """
        :param inputs: number of inputs
        :param hidden_units
        :param out_puts: number of outputs
        ...
        :param reg:
        """
        super(merge_IFM, self).__init__()
        # parameters
        self.reg = reg
        # layers
        self.before = nn.Linear(inputs, numpy_dim, bias=True)
        self.before_drop = nn.Dropout(dp_ratio)
        
        self.hidden1 = SineLayer(numpy_dim + numpy_dim, hidden_units[0], is_first=True, omega_0=first_omega_0)
        self.dropout1 = nn.Dropout(dp_ratio)
    ...
    def forward(self, x_finger, x_numpy):
        x_finger = self.before(x_finger)
        x_finger = F.relu(self.before_drop(x_finger))
        x_finger = self.before_drop(x_finger)
        
        x = torch.cat((x_finger, x_numpy), dim=1)
...
class merge_IFM_4(nn.Module):
    def __init__(self, inputs, hidden_units, outputs, d_out, sigma, dp_ratio, first_omega_0, hidden_omega_0, reg, numpy_dim):
        ...
        :param reg:
        """
        super(merge_IFM_4, self).__init__()
        # parameters
        self.reg = reg
        # layers
        
        self.hidden1 = SineLayer(inputs + numpy_dim, hidden_units[0], is_first=True, omega_0=first_omega_0)
        self.dropout1 = nn.Dropout(dp_ratio)
...
def collate_fn_merge(data_batch):
    Xs, dl_feature ,Ys, masks = map(list, zip(*data_batch))
    Xs = torch.stack(Xs, dim=0)
    dl_feature = torch.stack(dl_feature, dim=0)
    Ys = torch.stack(Ys, dim=0)
    masks = torch.stack(masks, dim=0)
    return Xs, dl_feature, Ys, masks
